...
class conv_net():
    ...
    def __init__(self,layers,conv_layers,fc_layers,conv_z_matrix,fil_size_matrix,
        ...
        self.input_pixels = tf.placeholder(tf.float32,shape=[input_x,input_y,input_z],name='input_pixels') # This is the input that will be fed externally
        ...
        self.cost_function_out = tf.subtract(self.cost_function_in,self.error)           #self.class_err_c#tf.add(self.class_err_c,tf.add(self.prob_err_c,tf.add(self.pos_err_c, self.size_err_c))) #tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=[self.AC_c],logits=[self.NN_c]))
    ...
    def execute(self, pixels, actual_output, gradient_val, cost_func_acm):
        ...
        A_c, NN_c, CF, BCE, OC, G, CW = self.sess.run \
        (
            [self.AC_c, self.NN_c, self.cost_function_out, self.error, self.output_correct,self.gradients, self.current_weights[6]],
            feed_dict={self.input_pixels: pixels, self.AC_OUTPUT: actual_output, self.gradient_val: gradient_val, self.cost_function_in: cost_func_acm}
        ...
        return A_c, NN_c, CF, BCE, OC, G, CW
